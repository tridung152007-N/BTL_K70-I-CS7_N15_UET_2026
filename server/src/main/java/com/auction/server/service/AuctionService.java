...
public class AuctionService implements AuctionSubject {
    ...
    public Auction payAuction(String auctionId, String bidderId, UserService userService) {
        lock.lock();
        try {
            Auction auction = auctionDAO.findById(auctionId)
                    .orElseThrow(() -> new IllegalArgumentException("Khong tim thay phien: " + auctionId));

            boolean timeEnded = auction.getEndTime() != null && !auction.getEndTime().isAfter(LocalDateTime.now());
            if (auction.getState() != AuctionState.FINISHED && !timeEnded) {
                throw new IllegalArgumentException("Chi thanh toan sau khi phien dau gia ket thuc");
            }
            if (auction.getCurrentLeaderId() == null || !auction.getCurrentLeaderId().equals(bidderId)) {
                throw new IllegalArgumentException("Chi nguoi thang dau gia moi duoc thanh toan");
            }
            if (auction.isPaid()) {
                throw new IllegalArgumentException("Phien nay da duoc thanh toan");
            }

            userService.pay(bidderId, auction.getCurrentPrice());
            auction.setPaid(true);
            if (timeEnded && auction.getState() != AuctionState.FINISHED) {
                auction.setState(AuctionState.FINISHED);
            }
            auctionDAO.update(auction);
            return auction;
